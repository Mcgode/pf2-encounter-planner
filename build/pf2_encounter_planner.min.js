...
let n = t.level - this.expectedLevel;
if (n > 4) return null;
n >= -4 && (e += t.amount * m[n.toString()] * (t.isComplex ? 5 : 1));
}
return e;
}
exportToJSON() {
...
for (let n of e.hazards) t.hazards.push(u.importFromJSON(n));
return t;
}
getNewHazardId() {
let e,
...
static importFromJSON(e) {
return new u(e.name, e.level, e.isComplex, e.amount, e.link, e.id);
}
}
const m = {
"-4": 2,
...
a = s.expectedLevel;
(s.expectedLevel = e),
(s.expectedPlayers = r.length + n.additionalNPCs),
...
u = s.expectedLevel;
(s.expectedLevel = p), (i = s.getEncounterXpPerPlayer()), (s.expectedLevel = u);
break;
default:
i = s.getEncounterXpPerPlayer();
}
