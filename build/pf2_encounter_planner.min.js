...
let n = t.level - this.expectedLevel;
if (n > 4) return null;
n >= -4 && (e += t.amount * h[n.toString()] * (t.isComplex ? 5 : 1));
}
return e;
}
exportToJSON() {
...
break;
case t.CUSTOM:
s.component = u.importFromJSON(e.component);
}
return s;
}
}
...
return new f(e.id, e.players, t.findElementById(e.element), e.levelUp);
}
}
class v {
constructor(e, t = []) {
...
let n = this.playerHistory[s.id];
n &&
n.last().xp >= 1e3 &&
(n.push({index: e, level: n.last().level + 1, xp: n.last().xp - 1e3}),
(t = !0));
}
t || this.errorEvents.push(n), this.xpChange.push(null);
...
? (this.errorEvents.push(n), (i = null))
...
this.xpChange.push(i), (i = i || 0);
for (let t of l)
this.playerHistory[t.id].push({index: e, xp: t.xp + i, level: t.level});
} else
console.log("No players for event " + n.element.name), this.errorEvents.push(n);
} else this.errorEvents.push(n);
e++;
}
}
}
...
addTimelineEvent(e, t, n = !1) {
let s,
r,
l = this.findElementById(t);
if (null == l) return null;
do {
(s = "event-" + Math.floor(Math.random() * Number.MAX_SAFE_INTEGER)),
(r = null != this.timelineEvents.find(e => e.id === s));
} while (r);
let i = new f(
s,
this.params.players.map(e => e.id),
l,
n
);
return this.timelineEvents.splice(e, 0, i), i;
}
static importFromJSON(e) {
let t = JSON.parse(e),
...
for (let e of t.timelineEvents) s.push(f.importFromJSON(e, n));
return (n.timeline = new v(n, s)), n;
}
